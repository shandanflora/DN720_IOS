...
public class UnibotCleanActivity {
    ...
    private MobileElement promptConfirm = null;
...
/*
    ...
    private boolean translatePrompt(Map<String, String> tranMap){
        String strLanguage = tranMap.get("language");
        boolean bpromptContent = promptContent.getText().trim().equalsIgnoreCase(tranMap.get("random_deebot_switch_task"));
        if (!bpromptContent) {
            TranslateErrorReport.getInstance().insetNewLine(
                    strLanguage, "UnibotClean", promptContent.getText(),
        ...
        if (!bpromptCancel) {
            TranslateErrorReport.getInstance().insetNewLine(
                    strLanguage, "UnibotClean", promptCancel.getText(),
                    tranMap.get("random_deebot_cancel"), "fail");
        }
        boolean bpromptConfirm = promptConfirm.getText().equalsIgnoreCase(tranMap.get("random_deebot_confirm"));
        if (!bpromptConfirm) {
            TranslateErrorReport.getInstance().insetNewLine(
                    strLanguage, "UnibotClean", promptConfirm.getText(),
    ...
    private boolean checkStatus(Map<String, String> tranMap){
        // auto to edge to spot to charge
